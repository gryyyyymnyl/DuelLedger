...
public class MatchStartDetector : IStateDetector
{
    ...
    private readonly (double x, double y, double w, double h) _tplRel = (0.00, 0.00, 1.00, 1.00);//0.30, 0.60, 0.45, 0.30

    /// <summary>
    /// 複数テンプレートをロード（ラベルはファイル名 stem を使用）
    /// </summary>
    public MatchStartDetector(IEnumerable<string> templatePaths)
    {
    ...
    public bool IsMatch(Mat screen, out double score, out OpenCvSharp.Point location)
    {
        ...
        Message = (string.Join(", ", BestLabelsInGroups)?.Contains("matchStart__1st") == true) ? "先行"
                        : (string.Join(", ", BestLabelsInGroups)?.Contains("matchStart__2nd") == true) ? "後攻"
