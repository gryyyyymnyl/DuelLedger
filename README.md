...
スナップショットと試合結果は PascalCase の JSON (`Format`, `SelfClass`, `StartAt`, `EndAt` など) として `out/current.json` へ原子的に書き出され、UI は `FileShare.ReadWrite | FileShare.Delete` で監視します。リネーム/作成/変更イベントで即座に状態が反映されます。
